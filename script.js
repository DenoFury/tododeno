...
let touchStartX = 0;
if(canvasWrapper) {
    canvasWrapper.addEventListener('touchstart', (e) => { if (e.touches.length > 0) { touchStartX = e.touches[0].screenX; } }, {passive: true});
    canvasWrapper.addEventListener('touchend', (e) => {
        if (e.changedTouches.length > 0) {
            let touchEndX = e.changedTouches[0].screenX;
            const notebook = notebooks.find(nb => nb.id === activeNotebookId);
            if (!notebook) return;
            if (touchEndX < touchStartX - 75 && currentPageIndex < notebook.pages.length - 1) { saveCanvasToNotebook(); currentPageIndex++; loadCurrentPage(); }
            if (touchEndX > touchStartX + 75 && currentPageIndex > 0) { saveCanvasToNotebook(); currentPageIndex--; loadCurrentPage(); }
        }
    }, {passive: true});
}
...
document.getElementById('btnSavePdf')?.addEventListener('click', async () => {
    ...
    if (safeColor === 'paper-yellow') { bgColor = '#fffdd0'; lineCol = '#e5cc99'; }
    if (safeColor === 'paper-black') { bgColor = '#1c1c1e'; lineCol = '#3a3a3c'; }
    ...
    for (let i = 0; i < notebook.pages.length; i++) {
        ...
        if (notebook.pages[i]) {
            const img = new Image(); img.src = notebook.pages[i];
            await new Promise(resolve => { img.onload = () => { tCtx.drawImage(img, 0, 0, 800, 600); resolve(); }; });
        }
